...
public class SystemController {
    ...
    public Result synthesis1(@RequestParam("bigUrl") String bigUrl, @RequestParam("smallUrl") String smallUrl,
                @RequestParam("caseId") String caseId, @RequestParam("abscissa") Integer abscissa, HttpServletRequest request,
                @RequestParam("ordinate") Integer ordinate, @RequestParam("fileName") String fileName) throws Exception {
        BufferedImage small = ImageHelper.loadImageUrl(smallUrl);
        BufferedImage big = ImageHelper.loadImageUrl(bigUrl);
        ...
        ImageHelper.writeImageLocal(outFile, ImageHelper.modifyImageTogether(small, big, abscissa, ordinate));
        return Result.success("合成成功！", fileName);
    }
          ...
          for (File f : list) {
              for (Object object : jsonArray) {
                  ...
                  if (fileName.contains(caseId)) {
                      ...
                      boolean b = f.renameTo(new File(file,fileUrl));
                      if (b) {
                        System.out.println(fileName + "  重命名成功！");
                      }
                  }
              }
          }
